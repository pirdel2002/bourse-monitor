...
export function buildIndicatorAnalysis(history,liveRow=null){
  ...
  const macdHist=macdLine.map((value,i)=>finite(value)&&finite(macdSignal[i])?value-macdSignal[i]:null),rsi=rsiSeries(closes),mfi=mfiSeries(candles),obv=obvSeries(candles),atr=atrSeries(candles),last=closes.at(-1);
  const bbwSeries=closes.map((_,index)=>{const window=closes.slice(Math.max(0,index-19),index+1);if(window.length<20)return null;const middle=mean(window);return middle?std(window)*4/middle:null;}),bbw=lastFinite(bbwSeries),bbwPrev=lastFinite(bbwSeries,1),bbwPrev2=lastFinite(bbwSeries,2),bbwHistory=bbwSeries.filter(finite).slice(-120),bbwPercentile=finite(bbw)&&bbwHistory.length>=20?bbwHistory.filter(value=>value<=bbw).length/bbwHistory.length*100:null;
  const volumeSma20=volumes.length>20?mean(volumes.slice(-21,-1)):volumes.length>=20?mean(volumes.slice(-20)):null;
  const ema20=lastFinite(ema20s),ema20Prev=lastFinite(ema20s,1),ema50=lastFinite(ema50s),ema50Prev=lastFinite(ema50s,1),rsi14=lastFinite(rsi),rsiPrev=lastFinite(rsi,1),mfi14=lastFinite(mfi),mfiPrev=lastFinite(mfi,1),macd_line=lastFinite(macdLine),macd_signal=lastFinite(macdSignal),macd_hist=lastFinite(macdHist),macd_hist_prev1=lastFinite(macdHist,1),macd_hist_prev2=lastFinite(macdHist,2),obvNow=lastFinite(obv),atr14=lastFinite(atr),atrPrev=lastFinite(atr,1),atrPrev2=lastFinite(atr,2),atr5=mean(atr.filter(finite).slice(-5));
  const high=n=>obv.length>n?Math.max(...obv.slice(-(n+1),-1)):null;
  const priorHigh=n=>candles.length>n?Math.max(...candles.slice(-(n+1),-1).map(x=>x.high)):null;
  const priorLow=n=>candles.length>n?Math.min(...candles.slice(-(n+1),-1).map(x=>x.low)):null;
  const rollingReturn=n=>candles.length>n&&Number(candles.at(-1-n)?.close)>0?(last/Number(candles.at(-1-n).close)-1)*100:null;
  const impulseWindow=candles.slice(-40);let impulseLow=Infinity,maxGain40=null;
  for(const candle of impulseWindow){impulseLow=Math.min(impulseLow,candle.low);if(impulseLow>0)maxGain40=Math.max(maxGain40??-Infinity,(candle.high/impulseLow-1)*100);}
