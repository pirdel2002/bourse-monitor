...
export class ApiQuota {
  ...
  reserve(endpoint,date=new Date()){
    ...
    const result=this.db.reserveApi(endpoint,[{type:'day',key:day,limit:limits.daily},{type:'5min',key:`${day}T${p.hour}:${five}`,limit:limits.fiveMinutes},{endpoint:'__global__',type:'global-5min',key:`${day}T${p.hour}:${five}`,limit:this.config.globalFiveMinutes}]);
