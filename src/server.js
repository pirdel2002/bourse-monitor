...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname==='/api/rules'&&req.method==='GET')return json(res,200,{rules:db.listRulesV2({symbol:url.searchParams.get('symbol')||''})});
    ...
    if(url.pathname==='/api/portfolio'&&req.method==='GET')return json(res,200,db.portfolioOverview());
    if(url.pathname==='/api/portfolio'&&req.method==='PUT'){const body=await readBody(req),symbol=String(body.symbol||'').trim(),quantity=Math.floor(localizedNumber(body.quantity)),avgPrice=localizedNumber(body.avgPrice??body.avg_price),buyFeePct=localizedNumber(body.buyFeePct??body.buy_fee_pct??1.262);if(!symbol)throw new Error('نماد سبد الزامی است.');if(!Number.isInteger(quantity)||quantity<=0)throw new Error('تعداد سهم باید عدد صحیح بزرگ‌تر از صفر باشد.');if(!Number.isFinite(avgPrice)||avgPrice<=0)throw new Error('میانگین خرید باید بزرگ‌تر از صفر باشد.');if(!Number.isFinite(buyFeePct)||buyFeePct<0||buyFeePct>10)throw new Error('درصد کارمزد خرید باید بین صفر و ۱۰ باشد.');const position=db.savePortfolioPosition({...body,symbol,quantity,avgPrice,buyFeePct});return json(res,200,{position});}
