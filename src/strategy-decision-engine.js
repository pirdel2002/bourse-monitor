...
export class StrategyDecisionEngine{
  ...
  async run({dryRun=false,offline=false,snapshot=null}={}){
    ...
    const configs=strategyRules.filter(rule=>rule.user_id!=null).map(rule=>({userId:Number(rule.user_id),symbol:rule.symbol,tier:rule.actionParams.tier||'RULE_WATCH',trigger:rule.expression,stop:rule.actionParams.stop??null,target1:rule.actionParams.target1??null,noChaseMax:rule.actionParams.noChaseMax??null,quantity:rule.actionParams.quantity??null,limitPrice:rule.actionParams.limitPrice??rule.actionParams.proposedPrice??null,allocatedToman:rule.actionParams.allocatedToman??null,ruleId:rule.ruleId,buyEnabled:rule.actionParams.buyEnabled!==false}));
    const positions=this.db.listUsers().filter(user=>user.active).flatMap(user=>this.db.portfolioPositions(user.id).map(position=>({...position,user_id:user.id})));
    ...
    const usageBefore=this.db.apiUsage(),rows=snapshot?.rows||(offline?this.db.allSymbolDetails():await this.marketData.marketRows()),market=snapshot?.market||{},bySymbol=new Map(rows.map(row=>[row.symbol,row])),contexts=snapshot?.contexts||new Map(),watchlist=[];
    for(const config of configs){const context=contexts.get(`${config.userId}:${config.symbol}`)||cachedContext(this.db,config.symbol,bySymbol.get(config.symbol),market,config.userId),stored=this.db.strategyState(LEGACY_STRATEGY_KEY,config.symbol,config.userId),account=this.db.getDataState(`account:cash:${config.userId}`),report=buyDecision(config,context,stored,account);report.userId=config.userId;if(snapshot?.riskOff&&report.finalState==='BUY_NOW'){report.finalState='BUY_CANDIDATE';report.reason+=' MARKET_RISK_OFF: یک سطح کاهش یافت.';}else if(snapshot?.riskOff&&report.finalState==='BUY_CANDIDATE'){report.finalState='TRIGGERED_WAIT_CONFIRMATION';report.reason+=' MARKET_RISK_OFF: یک سطح کاهش یافت.';}watchlist.push(report);if(!dryRun){const oldDecision=stored?.last_decision,changed=Boolean(stored)&&oldDecision!==report.finalState,actionable=['BUY_NOW','BUY_CANDIDATE'].includes(report.finalState),cooldown=45*60000,cooldownOk=!stored?.last_notified_at||Date.now()-Date.parse(stored.last_notified_at)>=cooldown;let notified=false;if(changed&&actionable&&cooldownOk){const sent=await sendTelegramMany(this.runtime.telegramTargets(config.userId),decisionMessage(report));notified=!sent.skipped;}this.db.saveStrategyState(LEGACY_STRATEGY_KEY,config.symbol,{state:report.finalState,buyEnabled:config.buyEnabled,triggerRuleId:config.ruleId,lastDecision:report.finalState,reason:report,triggeredAt:report.trigger==='active'?(stored?.triggered_at||new Date().toISOString()):stored?.triggered_at,confirmedAt:['BUY_NOW','BUY_CANDIDATE'].includes(report.finalState)?new Date().toISOString():stored?.confirmed_at,lastNotifiedAt:notified?new Date().toISOString():stored?.last_notified_at,lastNotifiedDay:notified?tehranDay():stored?.last_notified_day},config.userId);}}
    const portfolio=[];
    for(const position of positions){const userId=Number(position.user_id),context=contexts.get(`${userId}:${position.symbol}`)||cachedContext(this.db,position.symbol,bySymbol.get(position.symbol),market,userId),report=exitDecision(position,context);report.userId=userId;portfolio.push(report);if(!dryRun){this.db.updatePortfolioRisk(position.symbol,{highestPrice:report.highestSinceEntry,currentStop:report.currentStop,initialStop:report.initialStop},userId);const stored=this.db.strategyState(LEGACY_STRATEGY_KEY,position.symbol,userId),changed=Boolean(stored)&&stored.last_decision!==report.finalDecision,actionable=['SELL_ALL','STRUCTURAL_SELL_ALL','TAKE_PROFIT','REDUCE_POSITION'].includes(report.finalDecision),cooldown=15*60000,cooldownOk=!stored?.last_notified_at||Date.now()-Date.parse(stored.last_notified_at)>=cooldown;let notified=false;if(changed&&actionable&&cooldownOk){const sent=await sendTelegramMany(this.runtime.telegramTargets(userId),decisionMessage(report));notified=!sent.skipped;}const buyReport=watchlist.find(x=>x.userId===userId&&x.symbol===position.symbol),winning=!buyReport||statePriority[report.finalDecision]>statePriority[buyReport.finalState]?report.finalDecision:buyReport.finalState;this.db.saveStrategyState(LEGACY_STRATEGY_KEY,position.symbol,{state:winning,buyEnabled:true,lastDecision:winning,reason:report,lastNotifiedAt:notified?new Date().toISOString():stored?.last_notified_at,lastNotifiedDay:notified?tehranDay():stored?.last_notified_day},userId);}}
