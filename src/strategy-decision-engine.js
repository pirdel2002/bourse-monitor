...
import {REVERSAL_WATCH_STRATEGY,portfolioStructuralSupports,reversalWatchSymbols} from './reversal-watch-v1.js';
...
function exitDecision(position,context){
  ...
  const hardStop=stop.profit<=-8,positionStopHit=stop.price<=stop.current,support=portfolioStructuralSupports[position.symbol]??null,supportBroken=support!==null&&stop.price<support,severe=supportBroken&&Number(context.current?.volume_ratio_20)>=1.5&&Number(context.current?.real_money_flow)<0,structural=supportBroken&&(weaknessMatches>=1||severe);
  ...
  else if(structural){finalDecision='STRUCTURAL_SELL_ALL';exitRule=severe?'STRUCTURE_BREAK_SEVERE':'STRUCTURE_BREAK_CONFIRMED';}
...
export class StrategyDecisionEngine{
  ...
  async run({dryRun=false,offline=false,snapshot=null}={}){
    const usageBefore=this.db.apiUsage(),rows=snapshot?.rows||(offline?this.db.allSymbolDetails():await this.marketData.marketRows()),market=snapshot?.market||{},bySymbol=new Map(rows.map(row=>[row.symbol,row])),contexts=snapshot?.contexts||new Map(),watchlist=[],initialStates=new Map(this.db.strategyStates(REVERSAL_WATCH_STRATEGY).map(row=>[row.symbol,row]));
    const account=this.db.getDataState('account:cash');
    for(const config of reversalWatchSymbols){const context=contexts.get(config.symbol)||cachedContext(this.db,config.symbol,bySymbol.get(config.symbol),market),stored=initialStates.get(config.symbol)||null,report=buyDecision(config,context,stored,account);if(snapshot?.riskOff&&report.finalState==='BUY_NOW'){report.finalState='BUY_CANDIDATE';report.reason+=' MARKET_RISK_OFF: یک سطح کاهش یافت.';}else if(snapshot?.riskOff&&report.finalState==='BUY_CANDIDATE'){report.finalState='TRIGGERED_WAIT_CONFIRMATION';report.reason+=' MARKET_RISK_OFF: یک سطح کاهش یافت.';}watchlist.push(report);if(!dryRun){const oldDecision=stored?.last_decision,changed=Boolean(stored)&&oldDecision!==report.finalState,actionable=['BUY_NOW','BUY_CANDIDATE'].includes(report.finalState),cooldown=45*60000,cooldownOk=!stored?.last_notified_at||Date.now()-Date.parse(stored.last_notified_at)>=cooldown;let notified=false;if(changed&&actionable&&cooldownOk){const sent=await sendTelegramMany(this.runtime.telegramTargets(),decisionMessage(report));notified=!sent.skipped;}this.db.saveStrategyState(REVERSAL_WATCH_STRATEGY,config.symbol,{state:report.finalState,buyEnabled:config.symbol!=='فولاد',triggerRuleId:`REVERSAL_V1_TRIGGER_${config.symbol}`,lastDecision:report.finalState,reason:report,triggeredAt:report.trigger==='active'?(stored?.triggered_at||new Date().toISOString()):stored?.triggered_at,confirmedAt:['BUY_NOW','BUY_CANDIDATE'].includes(report.finalState)?new Date().toISOString():stored?.confirmed_at,lastNotifiedAt:notified?new Date().toISOString():stored?.last_notified_at,lastNotifiedDay:notified?tehranDay():stored?.last_notified_day});}}
    const portfolio=[];
    for(const position of this.db.portfolioPositions()){const context=contexts.get(position.symbol)||cachedContext(this.db,position.symbol,bySymbol.get(position.symbol),market),report=exitDecision(position,context);portfolio.push(report);if(!dryRun){this.db.updatePortfolioRisk(position.symbol,{highestPrice:report.highestSinceEntry,currentStop:report.currentStop,initialStop:report.initialStop});const stored=initialStates.get(position.symbol)||null,changed=Boolean(stored)&&stored.last_decision!==report.finalDecision,actionable=['SELL_ALL','STRUCTURAL_SELL_ALL','TAKE_PROFIT','REDUCE_POSITION'].includes(report.finalDecision),cooldown=15*60000,cooldownOk=!stored?.last_notified_at||Date.now()-Date.parse(stored.last_notified_at)>=cooldown;let notified=false;if(changed&&actionable&&cooldownOk){const sent=await sendTelegramMany(this.runtime.telegramTargets(),decisionMessage(report));notified=!sent.skipped;}const buyReport=watchlist.find(x=>x.symbol===position.symbol),winning=!buyReport||statePriority[report.finalDecision]>statePriority[buyReport.finalState]?report.finalDecision:buyReport.finalState;this.db.saveStrategyState(REVERSAL_WATCH_STRATEGY,position.symbol,{state:winning,buyEnabled:position.symbol!=='فولاد',lastDecision:winning,reason:report,lastNotifiedAt:notified?new Date().toISOString():stored?.last_notified_at,lastNotifiedDay:notified?tehranDay():stored?.last_notified_day});}}
    const usageAfter=this.db.apiUsage(),key=row=>`${row.endpoint}|${row.window_type}|${row.window_key}`,before=new Map(usageBefore.map(row=>[key(row),row.request_count])),apiCalls=usageAfter.reduce((sum,row)=>sum+Math.max(0,Number(row.request_count)-Number(before.get(key(row))||0)),0);this.lastRunAt=new Date().toISOString();this.lastError=null;return {strategy:REVERSAL_WATCH_STRATEGY,dryRun,offline,apiCalls,watchlist,portfolio};
  }
}
