...
test('builds the requested indicator context from cached candles and one live candle',()=>{const analysis=buildIndicatorAnalysis(candles,{date:'1405-07-03',lastPrice:1640,closePrice:1635,openPrice:1620,dayHigh:1650,dayLow:1610,volume:350000,tradeValue:500000000,tradeCount:900,buyVolumeReal:220000,buyCountReal:20,sellVolumeReal:120000,sellCountReal:20,buyerPower:220/120,hasBuyerPowerData:true});assert.equal(analysis.valid,true);for(const key of ['ema20','ema50','rsi14','macd_line','macd_signal','macd_hist','mfi14','obv','atr14','bbw','volume_sma20','volume_ratio_20'])assert.equal(Number.isFinite(analysis.context[key]),true,key);assert.equal(analysis.context.buyer_power>1.8,true);assert.equal(analysis.rows.length,13);});
