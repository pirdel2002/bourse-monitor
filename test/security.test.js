...
test('remembered sessions survive an auth service restart and are revocable',()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'bourse-auth-')),db=openDatabase(path.join(dir,'x.db')),first=new AuthService(db,'a-strong-password'),session=first.login('a-strong-password',true);assert.equal(session.expiresIn,30*86400);assert.equal(new AuthService(db,'a-strong-password').authorized(session.token).role,'SENIOR');first.logout(session.token);assert.equal(first.authorized(session.token),null);db.close();});
